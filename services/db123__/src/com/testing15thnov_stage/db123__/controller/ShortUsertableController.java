...
import com.testing15thnov_stage.db123__.ShortUsertable;
import com.testing15thnov_stage.db123__.service.ShortUsertableService;
...
public ShortUsertable createShortUsertable(@RequestPart("wm_data_json") ShortUsertable shortUsertable, @RequestPart(value = "profile", required = false) MultipartFile _profile) {
		LOGGER.debug("Create ShortUsertable with information: {}" , shortUsertable);

    shortUsertable.setProfile(WMMultipartUtils.toByteArray(_profile));
    ...
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public ShortUsertable getShortUsertable(@PathVariable("id") Short id) throws EntityNotFoundException {
    ...
    @ApiOperation(value = "Retrieves content for the given BLOB field in ShortUsertable instance" )
    @RequestMapping(value = "/{id}/content/{fieldName}", method = RequestMethod.GET, produces="application/octet-stream")
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public DownloadResponse getShortUsertableBLOBContent(@PathVariable("id") Short id, @PathVariable("fieldName") String fieldName, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, @RequestParam(value="download", defaultValue = "false") boolean download) {

        LOGGER.debug("Retrieves content for the given BLOB field {} in ShortUsertable instance" , fieldName);

        if(!WMRuntimeUtils.isLob(ShortUsertable.class, fieldName)) {
            throw new TypeMismatchException("Given field " + fieldName + " is not a valid BLOB type");
        }
        ShortUsertable shortUsertable = shortUsertableService.getById(id);

        return WMMultipartUtils.buildDownloadResponseForBlob(shortUsertable, fieldName, httpServletRequest, download);
    }

    @ApiOperation(value = "Updates the ShortUsertable instance associated with the given id.")
    @RequestMapping(value = "/{id:.+}", method = RequestMethod.PUT)
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public ShortUsertable editShortUsertable(@PathVariable("id") Short id, @RequestBody ShortUsertable shortUsertable) throws EntityNotFoundException {
    ...
    @ApiOperation(value = "Updates the ShortUsertable instance associated with the given id.This API should be used when ShortUsertable instance fields that require multipart data.") 
    @RequestMapping(value = "/{id:.+}", method = RequestMethod.POST, consumes = {"multipart/form-data"})
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public ShortUsertable editShortUsertable(@PathVariable("id") Short id, MultipartHttpServletRequest multipartHttpServletRequest) throws EntityNotFoundException {
        ShortUsertable newShortUsertable = WMMultipartUtils.toObject(multipartHttpServletRequest, ShortUsertable.class, "db123__");
        newShortUsertable.setUserId(id);

        ShortUsertable oldShortUsertable = shortUsertableService.getById(id);
        WMMultipartUtils.updateLobsContent(oldShortUsertable, newShortUsertable);
        LOGGER.debug("Updating ShortUsertable with information: {}" , newShortUsertable);

        return shortUsertableService.update(newShortUsertable);
    }

    @ApiOperation(value = "Deletes the ShortUsertable instance associated with the given id.")
    @RequestMapping(value = "/{id:.+}", method = RequestMethod.DELETE)
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public boolean deleteShortUsertable(@PathVariable("id") Short id) throws EntityNotFoundException {
    ...
    public Page<ShortUsertable> searchShortUsertablesByQueryFilters( Pageable pageable, @RequestBody QueryFilter[] queryFilters) {
        LOGGER.debug("Rendering ShortUsertables list");
