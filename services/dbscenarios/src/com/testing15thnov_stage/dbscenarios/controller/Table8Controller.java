...
import com.testing15thnov_stage.dbscenarios.Table8;
import com.testing15thnov_stage.dbscenarios.service.Table8Service;
...
public Table8 createTable8(@RequestPart("wm_data_json") Table8 table8, @RequestPart(value = "column4", required = false) MultipartFile _column4) {
    ...
    @ApiOperation(value = "Retrieves content for the given BLOB field in Table8 instance" )
    @RequestMapping(value = "/{id}/content/{fieldName}", method = RequestMethod.GET, produces="application/octet-stream")
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public DownloadResponse getTable8BLOBContent(@PathVariable("id") Integer id, @PathVariable("fieldName") String fieldName, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, @RequestParam(value="download", defaultValue = "false") boolean download) {

        LOGGER.debug("Retrieves content for the given BLOB field {} in Table8 instance" , fieldName);

        if(!WMRuntimeUtils.isLob(Table8.class, fieldName)) {
            throw new TypeMismatchException("Given field " + fieldName + " is not a valid BLOB type");
        }
        Table8 table8 = table8Service.getById(id);

        return WMMultipartUtils.buildDownloadResponseForBlob(table8, fieldName, httpServletRequest, download);
    }
    ...
    @ApiOperation(value = "Updates the Table8 instance associated with the given id.This API should be used when Table8 instance fields that require multipart data.") 
    @RequestMapping(value = "/{id:.+}", method = RequestMethod.POST, consumes = {"multipart/form-data"})
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public Table8 editTable8(@PathVariable("id") Integer id, MultipartHttpServletRequest multipartHttpServletRequest) throws EntityNotFoundException {
        Table8 newTable8 = WMMultipartUtils.toObject(multipartHttpServletRequest, Table8.class, "dbscenarios");
        newTable8.setId(id);

        Table8 oldTable8 = table8Service.getById(id);
        WMMultipartUtils.updateLobsContent(oldTable8, newTable8);
        LOGGER.debug("Updating Table8 with information: {}" , newTable8);

        return table8Service.update(newTable8);
    }
    ...
    @RequestMapping(value="/{id:.+}/table8sForColumn2", method=RequestMethod.GET)
    @ApiOperation(value = "Gets the table8sForColumn2 instance associated with the given id.")
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public Page<Table8> findAssociatedTable8sForColumn2(@PathVariable("id") Integer id, Pageable pageable) {

        LOGGER.debug("Fetching all associated table8sForColumn2");
        return table8Service.findAssociatedTable8sForColumn2(id, pageable);
    }
