...
import com.testing15thnov_stage.dbscenarios.service.ZeroAsPkService;
...
public ZeroAsPk createZeroAsPk(@RequestPart("wm_data_json") ZeroAsPk zeroAsPk, @RequestPart(value = "column3", required = false) MultipartFile _column3) {
		LOGGER.debug("Create ZeroAsPk with information: {}" , zeroAsPk);

    zeroAsPk.setColumn3(WMMultipartUtils.toByteArray(_column3));
    ...
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public ZeroAsPk getZeroAsPk(@PathVariable("id") Integer id) throws EntityNotFoundException {
    ...
    @ApiOperation(value = "Retrieves content for the given BLOB field in ZeroAsPk instance" )
    @RequestMapping(value = "/{id}/content/{fieldName}", method = RequestMethod.GET, produces="application/octet-stream")
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public DownloadResponse getZeroAsPkBLOBContent(@PathVariable("id") Integer id, @PathVariable("fieldName") String fieldName, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, @RequestParam(value="download", defaultValue = "false") boolean download) {

        LOGGER.debug("Retrieves content for the given BLOB field {} in ZeroAsPk instance" , fieldName);

        if(!WMRuntimeUtils.isLob(ZeroAsPk.class, fieldName)) {
            throw new TypeMismatchException("Given field " + fieldName + " is not a valid BLOB type");
        }
        ZeroAsPk zeroAsPk = zeroAsPkService.getById(id);

        return WMMultipartUtils.buildDownloadResponseForBlob(zeroAsPk, fieldName, httpServletRequest, download);
    }

    @ApiOperation(value = "Updates the ZeroAsPk instance associated with the given id.")
    @RequestMapping(value = "/{id:.+}", method = RequestMethod.PUT)
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public ZeroAsPk editZeroAsPk(@PathVariable("id") Integer id, @RequestBody ZeroAsPk zeroAsPk) throws EntityNotFoundException {
    ...
    @ApiOperation(value = "Updates the ZeroAsPk instance associated with the given id.This API should be used when ZeroAsPk instance fields that require multipart data.") 
    @RequestMapping(value = "/{id:.+}", method = RequestMethod.POST, consumes = {"multipart/form-data"})
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public ZeroAsPk editZeroAsPk(@PathVariable("id") Integer id, MultipartHttpServletRequest multipartHttpServletRequest) throws EntityNotFoundException {
        ZeroAsPk newZeroAsPk = WMMultipartUtils.toObject(multipartHttpServletRequest, ZeroAsPk.class, "dbscenarios");
        newZeroAsPk.setId(id);

        ZeroAsPk oldZeroAsPk = zeroAsPkService.getById(id);
        WMMultipartUtils.updateLobsContent(oldZeroAsPk, newZeroAsPk);
        LOGGER.debug("Updating ZeroAsPk with information: {}" , newZeroAsPk);

        return zeroAsPkService.update(newZeroAsPk);
    }

    @ApiOperation(value = "Deletes the ZeroAsPk instance associated with the given id.")
    @RequestMapping(value = "/{id:.+}", method = RequestMethod.DELETE)
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public boolean deleteZeroAsPk(@PathVariable("id") Integer id) throws EntityNotFoundException {
    ...
    public Page<ZeroAsPk> searchZeroAsPksByQueryFilters( Pageable pageable, @RequestBody QueryFilter[] queryFilters) {
        LOGGER.debug("Rendering ZeroAsPks list");
